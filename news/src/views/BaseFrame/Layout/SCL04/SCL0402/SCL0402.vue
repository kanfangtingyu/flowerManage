<template>
  <div class="scl04">
    <el-card :class="isCardShow()">
      <div slot="header">
        <span class="header-span">第２種</span>
      </div>
      ...
      <div
        v-else-if="totalCount > 0 && !hasError"
        class="out-space title-height"
      >
        ...
        <div class="all-row-height">
          <el-row
            ...
            :key="index"
          >
            <span :class="judgeAuthorityStyle()" @click="openMclsec(item)">{{
              item.contractId
            }}</span>
          </el-row>
        </div>
...
export default {
  ...
  data() {
    return {
      optionId: OPTION_ID.SCL0402A
    }
  },
  created() {
    this.cardId = CARDIDLIST.SCL0402
  }
}
</script>
<style scoped>
